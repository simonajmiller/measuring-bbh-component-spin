...
for pop in pops: 
    ...
    for nevent in nevents:
        ...
        # emcee output
        name = f_root+f"{model}/{date}_{model}_pop{pop}_{nevent}events.json"
        with open(name, 'r') as f:
            pop_data = json.load(f)
            emcee_data_pop[nevent+'events'] = pop_data 

        # weighted sample dicts corresponding to the above emcee samples
        name2 = f_root+f"{model}/{date}_{model}_{pop}_{nevent}events_reweighted_sampleDict.json"
        with open(name2) as f: 
            rw_event_data = json.load(f)
...
for pop_key in ['pop1', 'pop2', 'pop3']:
    ...
    for nevents in [70, 330]:
        ...
        emcee_data = all_emcee_data[pop_key][events_key]
        ...
        for run in np.arange(nCatalogs):
            ...
            sampleDict_rw = all_sampleDicts_rw[pop_key][events_key]
        ...
        # Add results to dict 
        pred_vs_obs_spins_pop[events_key] = {
            'predicted':{
...
# Save
with open(f_root+'pred_vs_obs_spins_dict_pp.json', "w") as f:
