...
for pop in pops: 
    ...
    for nevent in nevents:
        ...
        for posterior_key in posterior_keys:
            ...
            emcee_data_posterior = {}
            rw_data_posterior = {}
            
            for chirpmass_key in chirpmass_keys: 
                
                if posterior_key!='bilby_posterior':
                    name = f"{model}/{date}_{model}_pop{pop}_{nevent}events_{posterior_key}_{chirpmass_key}"
                else: 
                    name = f"{model}/{date}_{model}_pop{pop}_{nevent}events_{chirpmass_key}"

                # emcee output
                with open(f_root+name+".json", 'r') as f:
                    emcee_data_posterior[chirpmass_key] =  json.load(f)

                # weighted sample dicts corresponding to the above emcee samples
                with open(f_root+name+"_reweighted_sampleDict.json", 'r') as f:
                    rw_data_posterior[chirpmass_key] = json.load(f)
                    
            emcee_data_event[posterior_key] = emcee_data_posterior
            rw_event_data[posterior_key] = rw_data_posterior
...
# Load injectionDict
with open(f_root+"../PopulationInferenceInput/injectionDict_full_mass_range_analytic.json", 'r') as f: 
    ...
    for posterior_key in posterior_keys:
        ...
        for chirpmass_key in chirpmass_keys:  
            ...
            for nevents in nevents_keys:
                ...
                print(events_key, posterior_key, chirpmass_key)

                # Fetch emcee data for this population
                emcee_data = all_emcee_data[pop_key][events_key][posterior_key][chirpmass_key]
                ...
                for run in np.arange(nCatalogs):
                    ...
                    sampleDict_rw = all_sampleDicts_rw[pop_key][events_key][posterior_key][chirpmass_key]
            ...
            pred_vs_obs_spins_posterior[chirpmass_key] = pred_vs_obs_spins_pop_run
                
        pred_vs_obs_spins_pop[posterior_key] = pred_vs_obs_spins_posterior
